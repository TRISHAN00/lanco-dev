...
const MyComponent = () => {
    ...
    const filteredlist = getData?.detail?.data?.list?.filter((e) => e?.page_data?.slug === 'be-smart-with-lanco')?.[0];

    const banner = filteredlist?.sections.filter((e) => e?.section_data?.slug === 'inner-banner-be-smart-by-lanco')?.[0];
    const story = filteredlist?.sections.filter((e) => e?.section_data?.slug === 'step-into-the-future-of-living-by-integrating-smart-technologies-with-your-present')?.[0];
    const business = filteredlist?.sections.filter((e) => e?.section_data?.slug === 'be-smart-with-lanco-slider')?.[0];
    const video = filteredlist?.sections.filter((e) => e?.section_data?.slug === 'be-smart-lanco-video')?.[0];
    const product = filteredlist?.sections.filter((e) => e?.section_data?.slug === 'be-smart-lanco-products')?.[0];
