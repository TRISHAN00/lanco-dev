...
const MyComponent = () => {
    ...
    const filteredlist = getData?.detail?.data?.list?.filter((e) => e?.page_data?.slug === 'lanco-property-management')?.[0];

    const banner = filteredlist?.sections.filter((e) => e?.section_data?.slug === 'inner-banner-lanco-property-management')?.[0];
    const story = filteredlist?.sections.filter((e) => e?.section_data?.slug === 'proficient-property-management-your-success-our-expertise')?.[0];
    const business = filteredlist?.sections.filter((e) => e?.section_data?.slug === 'property-management-slider')?.[0];
    const video = filteredlist?.sections.filter((e) => e?.section_data?.slug === 'property-management-video')?.[0];
    const product = filteredlist?.sections.filter((e) => e?.section_data?.slug === 'business-services')?.[0];
