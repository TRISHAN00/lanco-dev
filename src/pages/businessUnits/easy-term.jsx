...
const MyComponent = () => {
    ...
    const filteredlist = getData?.detail?.data?.list?.filter((e) => e?.page_data?.slug === 'lanco-easy-term')?.[0];

    const banner = filteredlist?.sections.filter((e) => e?.section_data?.slug === 'inner-banner-lanco-easy-term')?.[0];
    const story = filteredlist?.sections.filter((e) => e?.section_data?.slug === 'turning-homeownership-dreams-into-reality-with-trust')?.[0];
    const business = filteredlist?.sections.filter((e) => e?.section_data?.slug === 'business-slider-lanco-easy')?.[0];
    const video = filteredlist?.sections.filter((e) => e?.section_data?.slug === 'lanco-easy-team-video')?.[0];
    const product = filteredlist?.sections.filter((e) => e?.section_data?.slug === 'loan-offerings')?.[0];
