...
const MyComponent = () => {
    ...
    const filteredlist = getData?.detail?.data?.list?.filter((e) => e?.page_data?.slug === 'line-architects-ltd')?.[0];

    const banner = filteredlist?.sections.filter((e) => e?.section_data?.slug === 'inner-banner-line-architects-ltd')?.[0];
    const story = filteredlist?.sections.filter((e) => e?.section_data?.slug === 'design-your-unique-living-in-a-residence-uniquely-designed-for-you')?.[0];
    const business = filteredlist?.sections.filter((e) => e?.section_data?.slug === 'line-architects-ltd-slider')?.[0];
    const video = filteredlist?.sections.filter((e) => e?.section_data?.slug === 'line-architects-video')?.[0];
    const product = filteredlist?.sections.filter((e) => e?.section_data?.slug === 'architectural-solutions')?.[0];
