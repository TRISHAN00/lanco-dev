...
const MyComponent = () => {
    ...
    const filteredlist = getData?.detail?.data?.list?.filter((e) => e?.page_data?.slug === 'lanco-brokerage')?.[0];

    const banner = filteredlist?.sections.filter((e) => e?.section_data?.slug === 'inner-banner-lanco-brokerage')?.[0];
    const story = filteredlist?.sections.filter((e) => e?.section_data?.slug === 'elevate-your-property-value-with-our-data-backed-assurance')?.[0];
    const business = filteredlist?.sections.filter((e) => e?.section_data?.slug === 'business-slider-lanco-brokerage')?.[0];
    const video = filteredlist?.sections.filter((e) => e?.section_data?.slug === 'lanco-brokerage-video')?.[0];
    const product = filteredlist?.sections.filter((e) => e?.section_data?.slug === 'brokerage-offerings')?.[0];
