...
const Video = ({offset, padding, data}) => {
    ...
    return (
        ...
        <StyledVideo data-scroll-container offset={offset}
                     className={`video_body ${padding ? padding : ''} `}>
            <ModalVideo channel='youtube' isOpen={open} videoId={data?.data?.link} onClose={() => handelOpen(false, '')}/>

            <Container>
                <Row>
                    <Col md={{span: 12}}>
                        <div onClick={() => handelOpen(true, `c3cWkjcO0EE`)} className="image_video_wrap">
                            <Img src={data?.images?.list?.filter(image => image?.short_title === 'video')?.[0]?.full_path}/>
                            <div className="play">
                                <svg xmlns="http://www.w3.org/2000/svg" width="100" height="100" viewBox="0 0 100 100">
                                    ...
                                    <path id="Polygon_1" data-name="Polygon 1" d="M10,0,20,15H0Z"
                                          transform="translate(58.5 40) rotate(90)" fill="#565440"/>
                                </svg>
